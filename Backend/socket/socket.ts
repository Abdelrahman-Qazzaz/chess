  ...
  onConnection: (socket: any) => {
    ...
    socket.on(
      ...
      (userProfile: { name: string; avatar: string }) => {
        inQueue.push({ ...userProfile, socketID: socket.id });
        if (inQueue.length > 1) {
          const opponentSocketID = inQueue.find(
    ...
    socket.on("Make a move", (newFEN: string) => {
      const targetRoom = rooms.find((room) =>
        room.players.find((player: any) => player.socketID == socket.id)
      );

      const targetPlayer =
        targetRoom.players[0].socketID == socket.id
          ? targetRoom.players[1]
          : targetRoom.players[0];
      const otherPlayer =
        targetPlayer === targetRoom.players[1]
          ? targetRoom.players[0]
          : targetRoom.players[1];
    ...
    socket.on("Game over", async (opponent: any, you: any, method: string) => {
      const targetRoom = rooms.find((room) =>
        room.players.find(
          (player: any) =>
      ...
      const sockets = await serverExports.io.fetchSockets();
      sockets
        .find((socket: any) => socket.id === opponent.socketID)
        .leave(targetRoom);
      sockets
        .find((socket: any) => socket.id === you.socketID)
        .leave(targetRoom);
      const filteredRooms = rooms.filter(
        (room) =>
          room.players.filter((player: any) => player.socketID === socket.id)
            .length === 0
      );
    ...
    socket.on(
      ...
      async (user1_data: any, user2_data: any) => {
        let room = {
    ...
    socket.on("disconnect", async () => {
      ...
      if (targetRoom) {
        ...
        for (const player of targetRoom.players) {
          const otherPlayerSocket = sockets.find(
            (socket: any) => socket.id === player.socketID
          );
...
function removeFromQueue(target: any) {
  const filteredQueue = inQueue.filter(
    (user) => user.socketID != target.socketID
  );
